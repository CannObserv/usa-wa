...
packages/
  ...
  usa-wa-adapter-pdc/                 — Layer 3: WA PDC (Public Disclosure Commission) SODA source
    src/usa_wa_adapter_pdc/
      ...
      normalize/pdc_span_emit.py — **identifier-only since #101**: `emit_pdc_identifiers`, the idempotent `person_wa_pdc` child-identifier upsert (per-Person, not per-tenure). The House Position **seat** emission moved to `usa_wa_adapter_sos.house_span_emit` when SOS became the seat authority (#101)
