...
packages/
  ...
  usa-wa-adapter-pdc/                 — Layer 3: WA PDC (Public Disclosure Commission) SODA source
    src/usa_wa_adapter_pdc/
      ...
      normalize/pdc_observations.py — **pure** Phase B projectors (#79): `build_house_position_observations` (winners → House Position `Observation`s + `person_wa_pdc` links, reusing the #69 match + #74 mid-biennium mover inference, era-matched by the caller; inferred seats carry no id + are tracked in `inferred_keys`; per-cohort coverage `summary`) and `build_senate_identity_links` (identifier-only Senate #75 links + robustness tally). Replaces the retired per-biennium `normalize_house_positions`/`normalize_senate_identities`/`persons.py` (a per-cohort normalize can't build a cross-year span, #79). **`position_fallback` (#100)**: PDC's `Campaign Finance Summary` dataset **omits the House `position` before the 2018 election** (proven in #98 — every pre-2018 House winner was `incomplete`), so a matched member with no PDC position now consults an injected `PositionFallback` (`(ld, folded_last, party_slug) → qualifier`, supplied by `usa-wa-adapter-sos` from the SOS filing archive) before falling to the new `missing_position` tally; `None` (the daily 2018+ path) keeps the pure PDC behaviour byte-for-byte (position-less → `incomplete`). `build_pdc_spans` takes an optional `house_position_fallback: year → PositionFallback|None` and threads it per cohort — the callable is typed here so **PDC keeps no SOS dependency**; the SOS package injects it
      normalize/pdc_span_emit.py — binds the House Position spans to the generic `span_emit` with the **source split** (`person_source=usa_wa_legislature`, `assignment_source=usa_wa_pdc` — PDC is the Position authority, the Person is WSL's): resolve the `state_representative` seat Role from the span discriminator, cite `house-winners:<Y>` per biennium; `emit_pdc_identifiers` is the idempotent `person_wa_pdc` upsert (per-Person, not per-tenure)
      ...
      build_pdc_spans.py — **Phase B** (#79, `python -m usa_wa_adapter_pdc.build_pdc_spans`): the #75 fix. Reads every archived winner cohort offline, pairs each with the roster of the biennium it **seated** (`[Y+1, Y+2]`, archive-first from the WSL sponsor archive — a 2012 cohort matches 2013-14, not current), projects observations + links, merges House observations across years into `usa_wa_pdc` Position seat **spans** + `person_wa_pdc` identifiers (Senate is identifier-only). Empty cohorts skipped. `restrict_to_biennium` scopes the daily re-drive to current members (each keeping full span history). **Depends on #77** (Persons + sponsor archive) for historical value — a pre-#77 winner's Person is absent so its span is skipped (logged, correct)
      ...
      refresh.py      — `python -m usa_wa_adapter_pdc.refresh`: daily cycle, **span-based (#79)**. Resolves the biennium (USA_WA_BIENNIUM override, else current; non-current logs `pdc_refresh_noncurrent_biennium`), archives the current cohorts via `archive_only` (forced past TTL for daily determinism), then re-drives `build_pdc_spans` scoped to the current biennium (House Position seat **spans** — current biennium = open end — + Senate `person_wa_pdc` links). The era roster is read archive-first from the WSL sponsor archive (`sponsors:<biennium>`, written by the WSL refresh, which runs first; live `GetSponsors` fallback for an un-archived biennium). Runs **after** the WSL refresh (its Persons must exist). The daily path passes **no** SOS fallback (current House carries PDC positions); the #100 SOS fill is a separate backfill (`usa-wa-adapter-sos`)
  usa-wa-adapter-sos/                 — Layer 3: WA Secretary of State (votewa) source — House Position backfill (#100)
    src/usa_wa_adapter_sos/
      transport.py    — `SOSClient`: async `httpx` reader for the votewa candidate-filing export (`eledataweb.votewa.gov/Candidates/ExportToExcel?electionDate=<YYYYMM>&countyCode=xx`) — despite the name serves **CSV** (`text/csv`), one row per candidate carrying `RaceName` (`State Representative Pos. 1/2`), `RaceJurisdictionName` (`Legislative District N`), `BallotName`, `PartyName` + contact/candidacy detail (`Email`/`MailingAddress`/`Phone`/`FilingDate`/`IsWithdrawn`, for #99). `fetch_whofiled(election_year)` returns `WireFetch` (pristine CSV bytes archived + hashed #54 + decoded rows); `parse_whofiled` is the offline re-parser (#56, stdlib `csv`, BOM-tolerant). Central courtesy min-interval gate `_SOS_LIMITER` (the #77 pattern) every GET passes through — env `USA_WA_SOS_MIN_REQUEST_INTERVAL` (default 1.0s, 0 disables), `configure_sos_rate_limit()` maps a harvest's `--pause-seconds`. `general_election_date(year)` → `<year>11` (WA general is November)
      ...
      sos_cohort.py   — `SosFilingCohortProvider` (Phase B): archive-first `{election_year: {LD: [HouseFiling]}}` re-parsed offline from each `sos-whofiled:` RawPayload (joins `RawPayload` so "latest" = latest payload-bearing event, the #82 lesson; memoized). `fallback_factory()` returns the sync `year → PositionFallback|None` the PDC builder injects — a year with no archived cohort → `None` (no fallback)
      harvest_sos.py  — **Phase A** (`python -m usa_wa_adapter_sos.harvest_sos`): sweep even general-election years (floor **2008**, the PDC winner floor) → archive `sos-whofiled:<YYYYMM>` via `archive_only` (no normalize). Central pacing via `--pause-seconds`; closed years cache-hit on re-run; `--dry-run`/`--force`
      build_sos_house_spans.py — **Phase B** (`python -m usa_wa_adapter_sos.build_sos_house_spans`): runs the PDC `build_pdc_spans` with the votewa fallback wired in — one coherent House rebuild (PDC positions 2018+, SOS fallback 2008–2016). Dependency is **one-directional** — the PDC builder takes an injected callable and never imports this package. Depends on the SOS archive (Phase A) + PDC winner archive + WSL sponsor archive/Persons (#77); run in the same window, **sidecar paused**. `--biennium` scopes the rebuild to a biennium's current members (each keeping full history); `--dry-run`. **⚠ NOT prod-safe standalone yet (#100 CR / #101)**: a member serving *across the 2018 boundary* builds an **open** deep span here (`…:2017-18`), but the daily `usa_wa_adapter_pdc.refresh` rebuilds House spans **without** this fallback → a shallow `…:2019-20` span, and the stale-span sweep closes the un-asserted deep one within ~24h — truncating the backfill for continuing members. Do **not** run the historical backfill in prod until the **#101 re-partition** (WSL+SOS-primary House Position, PDC identifier-only, symmetric with the Senate) lands; a full-history `--dry-run` is safe for inspection
...
| PDC refresh (daily) | oneshot + timer | — | `systemctl` (`usa-wa-pdc-refresh.timer` → `.service`; 06:30 UTC, #69; span-based since #79). Archives the current winner cohorts + re-drives the span builder → House `state_representative` Position seat **spans** + Senate `person_wa_pdc` links. Ordered after the WSL refresh (binds onto its House Persons + sponsor archive) |
...
| `python -m usa_wa_adapter_pdc.build_pdc_spans` | Era-matched House Position seat spans + identifiers, Phase B (#79) |
| `python -m usa_wa_adapter_pdc.migrate_pdc_spans` | Retire pre-#79 per-biennium PDC House rows onto spans (#79) |
| `python -m usa_wa_adapter_sos.harvest_sos` | Archive WA SOS votewa filing cohorts — Phase A (#100) |
| `python -m usa_wa_adapter_sos.build_sos_house_spans` | Fill pre-2018 House Position seats via SOS positions, Phase B (#100) |
