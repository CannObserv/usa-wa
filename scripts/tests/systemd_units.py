...
# systemd size suffixes → bytes. A bare number is bytes; `K`/`M`/`G`/`T` are
# powers of 1024 and each accepts a trailing `B` (`512M` and `512MB` are the same
# quantity to systemd, unlike SI usage elsewhere). `infinity` is systemd's
# sentinel for "no limit", which for a *reservation* means every page is
# protected — parsed as such rather than rejected.
_SIZE_UNIT_BYTES = {
    "": 1,
    "b": 1,
    "k": 1024,
...
def parse_bytes(value: str) -> float:
    ...
    if unit not in _SIZE_UNIT_BYTES:
        raise ValueError(f"unrecognized systemd size unit {unit!r} in {value!r}")
    return int(number) * _SIZE_UNIT_BYTES[unit]
