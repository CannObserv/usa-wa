...
def _tracked_docs(project_root: Path) -> list[str]:
    """Tracked Markdown at the repo root or under ``docs/``, in git's own order.
    ...
    out = subprocess.run(  # noqa: S603 — fixed argv, no shell, no user input
        ["git", "-c", "core.quotePath=false", "ls-files", "--", "*.md", "docs"],  # noqa: S607
        cwd=project_root,
        capture_output=True,
        text=True,
        timeout=30,
...
def findings(project_root: Path) -> list[str]:
    ...
    docs = _tracked_docs(project_root)
    ...
    # An opt-out nobody revisits is a blindfold. Both ways it rots leave an entry
    # that suppresses nothing while reading like a considered decision.
    tracked = set(docs)
    for entry in exempt:
        if entry not in tracked:
            out.append(f"stale exemption ({EXEMPT_FILE}): {entry} is not a tracked doc")
        elif _covered(entry, declared):
