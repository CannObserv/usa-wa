"""Cassette-replayed transport tests for the member pulls (P1b step 1, #82).

Replays recorded SOAP envelopes for ``SponsorService.GetSponsors`` and
``CommitteeService.GetCommitteeMembers`` — no live WSL. These pin the live ``Member`` field
names and, critically, the **per-endpoint party encoding** the sponsor normalizer must
reconcile: single letters (``"R"``/``"D"``) on the sponsor endpoint vs full words
(``"Democrat"``/``"Republican"``) on the committee endpoint (step 0 finding). The round-trip
tests prove the offline re-parsers recover the same records as the live pull (the #56 cache
path can't drift from #54's archival parse).

The archival ``GetActiveCommitteeMembers`` pair was retired in #82 (``GetCommitteeMembers``
keyed by the current biennium returns the identical set), so only the historical op is
archived — and only it needs cassettes.
"""
...
async def test_committee_members_party_is_full_word(wsl_vcr):
    ...
    the sponsor endpoint's ``D``/``R`` (the encoding split ``canonicalize_party`` reconciles).

    Pinned on the *historical* op since #82 retired the archival active-members pull; the
    encoding is a property of CommitteeService, not of which roster op you call."""
