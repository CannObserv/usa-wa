...
JOB_SLUG = "wsl-committee-member-span-migrate"
...
async def _migrate_job(ctx: JobContext) -> MigrationResult:
    ...
    return await migrate_committee_spans(ctx.require_session())


def main(argv: list[str] | None = None) -> int:
    """Run the membership span migration under the **owner** role.

    Retiring a stranded row deletes its citations, which the app role is REVOKEd on the
    provenance ledger (#54) — same contract as ``sponsors.migrate_spans``. Exit ``0``
    clean · ``1`` failed · ``2`` config (``DATABASE_URL_OWNER`` unset).
    """
    return run_job(
        JOB_SLUG,
        _migrate_job,
        argv=argv,
        prog="python -m usa_wa_adapter_legislature.membership.migrate_spans",
