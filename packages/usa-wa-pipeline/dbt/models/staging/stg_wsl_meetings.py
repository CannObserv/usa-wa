"""stg_wsl_meetings: thin dbt adapter over usa_wa_pipeline.staging.wsl.meeting_rows (#306).
...
def model(dbt, session):
    ...
    rows = wsl.meeting_rows(RawStore(get_raw_root(), "usa_wa_legislature"))
    return typed_relation(session, rows, wsl.MEETING_SCHEMA)
