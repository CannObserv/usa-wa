...
detecting is ours drifting from the tier that minted these keys (and published
them, until #314), and only a probe with a session can see it.
...
async def run_parity(
    ...
    *,
    ...
    # The ROLE dimension, against the tier that minted these keys (CR 77). The
    # dbt `assignments_name_a_role` test cannot catch a fork: `roles` is
    # generated by iterating `assignments` through the same `role_for_span`, so
