...
5. **Builder integration.** Wire the overlay into `build_sponsor_spans`, `usa_wa_facts_seats.house.build`, and `harvest_committee_member_spans` (fetch current non-superseded events, apply between projection and emit, thread the operator `citation_target` so affected Assignments cite the attestation). Verify each builder end-to-end on the LD5 fixtures.
