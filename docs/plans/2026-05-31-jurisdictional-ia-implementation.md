...
1. **File the upstream CannObserv/power-map feature request EARLY.** Title: "Jurisdiction entity type — extend identity model with bounded political/administrative areas." Body: cross-link to the design spec; summarize Sections 1 + 3 (PM data model + API contract); call out dependencies on #162 + #164; list the asks (slug convention confirmation, `type` vocabulary, relationship-type vocabulary). Framed to invite PM-side design review before usa-wa locks the schema. **Reordered from step 7 (2026-05-31)** — early filing surfaces PM-side feedback while local work is still cheap to revise. **Verifiable when:** the issue exists in CannObserv/power-map, references the design spec, and links back from `usa-wa#3`.
