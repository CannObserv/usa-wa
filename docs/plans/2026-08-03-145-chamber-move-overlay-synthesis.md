...
- **`house.build` is unrestricted and the daily SOS refresh runs it restricted-to-current.** The mover-signal + synthesis must behave identically in both — verify the daily restricted rebuild doesn't synthesize a historical mover (the mover-gate + `biennium(effective_date)` keying should confine it, but it needs an explicit test, mirroring the #119 open-synth guard).
- **Stale (non-mover) event members remain in `keep_ids`.** If a future stale case perturbs the elimination the way movers did, it will need the same treatment; out of scope here but worth a note in the code.
- **PM propagation during remediation.** Even with zero superseded rows, the re-dated + synthesized spans sync to PM. Confirm the mover's synthesized closed House span and re-dated backfiller land as clean in-place updates (biennium-keyed `source_id`, anchored) — no re-anchor. Consider pausing the sidecar per-tranche as a belt-and-suspenders until the no-churn property is proven once.
- **Backfiller dates for pre-2008 / obscure cases** may be imprecise; where a backfiller start can't be dated, that specific move stays biennium-granular (leave it) rather than guessing.
