...
5. ✅ **Backfiller research.** Folded into the batch research (`research_results.md`/`gh145_table.md`) — every distinct-member House-Position backfiller is dated (high confidence) and WSL Person ids resolved (2026-08-04). The House-Position sweep conflicts are **7 distinct-member cases** (4 true chamber-moves: LD34'09, LD46'11, LD18'11, LD28'13 + 3 simple House vacancies: LD15'09, LD49'11, LD9'15), NOT ~20 — the plan's earlier "~20" over-counted by including the **self-duplicate** artifacts (Ortiz-Self/Young/Robinson/Walkinshaw/Muri `X/X`), which are superseded-span build artifacts for a `house.build` + `house.migrate` collapse, not date-research or event cases. Full specs (ids + dates + reasons + evidence): scratchpad `chamber_move_specs.md`.
