...
3. **House builder.** In `house/build.py`, change `keep_ids = event_members − movers_by_biennium[b]` to `keep_ids = stale_exempt_members(events, b) − movers_by_biennium[b]`. It calls the *identical* proven helper the same way; coverage = the helper unit tests (scoping) + the sponsor integration test (wiring, RED-verified) + the existing 49-test house suite (non-regression) + the live re-proof (house end-to-end). A dedicated house-build ghost-extension test needs a heavy two-biennium ballot+committee fixture for no marginal assurance over the shared-helper proof, so it is deliberately not added.
