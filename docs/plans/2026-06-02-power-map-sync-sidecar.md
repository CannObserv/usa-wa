...
6. **Scaffold `usa-wa-sync-powermap` binding.** **5** concrete `EntityDescriptor`s (jurisdictions + canonical.{persons,organizations,roles,assignments}) — entity events are NOT a descriptor (sub-resource of person/org, per the 2026-06-05 PM #178 reassessment). Per-type `to_observation`/`upsert_from_pm`/`last_updated`; `write_enabled=true` for jurisdictions (others per appetite — all endpoints live). All `read_source="feed"` (feed covers all five) with reconcile backstop. Notes: assignment descriptor `entity_type="role_assignment"` (feed discriminator) though routes are `/assignments`; assignment `to_observation` keys on `person_id`+`role_id` PM ids → **sync ordering**: persons & roles before assignments. Add the `descriptor.fetch_record(client, pm_id)` seam (default → `client.get_entity`; person/org override to also pull `/{…}/{id}/events`) and refactor `engine.process_feed` to call it. Person/org event handling is a no-op hook until step 6b. Scope config (`slug_prefix=usa-wa`). Verify: descriptor unit tests per entity (payload mapping, anchor set, LWW field).

   6b. **(Deferred) Entity-event sync.** Refine `canonical.entity_events` to PM's `ObservationEventItem` shape (granular `event_year/month/day/hour/...`, `event_type_slug` XOR id, `event_place_text`, `visibility`, `linked_entity`), then wire the person/org `fetch_record` events hook + embed `events[]` in their observations. Tracked as a follow-up issue; out of the MVP increment.
7. **Daemon entrypoint + systemd unit.** `__main__` async daemon: `configure_logging()` once, build descriptors, run `SyncEngine` loops concurrently. `deploy/usa-wa-sync-powermap.service` (own lifecycle, env loading). **Per-cycle exception isolation (CR #13):** wrap each loop cycle in try/except + `session.rollback()` so a propagating non-transient error (now intentionally un-swallowed by the outbox worker) doesn't crash the daemon or poison the transaction. **Commit boundary (usa-wa#8):** default to per-entry commit with a configurable chunk size. Verify: daemon starts/stops cleanly on dev; a poison entry is isolated, not fatal; unit file lints.
...
9. **File PM coordination issues — DONE (frontloaded 2026-06-02).** Validated against PM source; filed [power-map#176](https://github.com/CannObserv/power-map/issues/176) (roles API), [#177](https://github.com/CannObserv/power-map/issues/177) (assignments API), [#178](https://github.com/CannObserv/power-map/issues/178) (entity-events public router), [#179](https://github.com/CannObserv/power-map/issues/179) (changes-feed jurisdiction coverage). Auth confirmed `X-API-Key`. **Validated reality refines steps 4/6:** jurisdictions are NOT on the `/changes` feed (read via full-reconcile primary); roles/assignments/entity_events have NO public surface (read or write) — their descriptors ship fully dormant (`read_source=none`, `write_enabled=false`), not just write-dormant.
