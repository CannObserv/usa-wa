...
- **power-map#322 (event void/retract) — SHIPPED (v0.14.2), client re-regenerated.** `op:"observe"|"retract"` on the same sub-resource; a re-link correction is create-new + retract-old in one partial-success batch (`retracted` disposition, archives not deletes), no admin step. C2/C3 implement corrections this way. A retract is *authoritative* — re-observing retracted content auto-attaches to the archived row, never resurrects (CR2 outcome), so the producer's no-op gate must treat a retracted anchor as terminal, not re-emit. **Live emission (step 8) waits on power-map's pending prod restart of #321/#322**; all code/tests are unblocked (built against the client + doubles).
