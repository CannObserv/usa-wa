...
What it surfaces: a declared-but-unindexed (or stale) context artifact **by name**, a `codebase_health` problem, a FAILED or INCOMPLETE last operation, and the graph edge-yield gate. That last one fired here daily until the file-dependency graph's repair in SocratiCode 1.13.0 ([`docs/SOCRATICODE.md`](SOCRATICODE.md)); it is quiet now, so a firing gate is a new finding — check the engine version first.
