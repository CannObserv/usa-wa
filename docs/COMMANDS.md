...
| `python -m usa_wa_pipeline.anchor_export` | PM crosswalk seed: every `pm_*` anchor as base32 pairs + manifest (#312), and materialized into the pipeline duckdb as `pm_anchors` for publication (#354). Read-only on Postgres; runs nightly before `publish` |
