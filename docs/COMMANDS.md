...
| `python -m usa_wa_pipeline.anchor_export` | PM crosswalk seed: every `pm_*` anchor as base32 pairs (#312), materialized into the pipeline duckdb as `pm_anchors` for publication (#354). Read-only on Postgres, **replaces** the duckdb table — point `--db` at scratch for ad-hoc runs. Runs nightly before `publish` |
