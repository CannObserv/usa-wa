...
| `python -m usa_wa_pipeline.registrar` | Cluster `proposed_links` plus a singleton per staged WSL sponsor (#403) (union-find) and apply the registry decision table (#308); also registers orgs (staged committees + meeting refs + structural orgs) and roles (the conformed dimension, #313) as singleton clusters; `--dry-run` previews; exit 4 = conflicts to triage |
