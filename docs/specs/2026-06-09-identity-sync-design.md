...
- **`q` name filtering → FTS** — [power-map#199](https://github.com/CannObserv/power-map/issues/199) (ILIKE, 2026-06-11) then [power-map#201](https://github.com/CannObserv/power-map/issues/201) (**FTS, 2026-06-14**) made `orgs/search` / `people/search` `q` a server-side `@@ plainto_tsquery` — word-token matching that folds punctuation, `&`, and word order (`pm_simple`), plus accents for people (`pm_unaccent_simple`). Cascade step 2 is now a **single FTS query** (`q` + `jurisdiction`) confirmed by `normalize_name` equality; FTS subsumes the `&`/punctuation cases, so the earlier ILIKE **cohort-scan fallback was removed** (`_cohort_candidates`/pagination/`_MAX_CANDIDATES` deleted). `normalize_name` was fixed to **unaccent** (NFKD + strip diacritics) to mirror `pm_unaccent_simple` — previously its ASCII-only collapse shredded accented names (`"José"` → `"jos "`), rejecting the accent matches FTS surfaces. Roles/jurisdictions got `search_tsv` columns but **no public search endpoint** (`/roles/search`, `/jurisdictions/search` → 404), so their caller surface is unchanged (roles match structurally, jurisdictions by slug). **Precision caveat:** the search response exposes no relevance score, so the confirm stays strict `normalize_name` equality — FTS improves candidate recall/ordering but we still only anchor on exact normalized matches (a producer wrong-anchor corrupts identity; a missed match is a mergeable dup).
