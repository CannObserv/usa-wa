...
- **`q` name filtering** — [power-map#199](https://github.com/CannObserv/power-map/issues/199) **shipped (2026-06-11)**: `orgs/search?q=<name>&jurisdiction=<slug>` now applies a server-side ILIKE on name/acronym/variants (people's `q` always did). Cascade step 2 uses it as a **q-narrowed fast path**, confirmed by `normalize_name` equality. #199 explicitly **deferred** PM-side `&`→`and`/punctuation folding, so a normalization-variant name can ILIKE-miss → the cascade **falls back to the full cohort scan** (paginated, 50/page via `search_entities`' `offset` cursor). The enumeration collapses away entirely only once PM ships server-side normalization.
