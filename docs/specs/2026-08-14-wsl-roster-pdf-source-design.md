...
| `Cit.` | Citizen | 1 | 1899 | Senate |
| `S` | Socialist | 1 | 1913 | Senate |

Expansions are the source's own, from the per-session party-division tables. Note `P.P.` and `Pop.` name the same historical movement but are **listed separately within a single session** (1897 Senate: `Populist 7`, `People's Party 8`), so collapsing them would make those tallies unreproducible — deferred to power-map#442 rather than decided locally. `Ind` (one 1889 member) stays excluded: `canonicalize_party` maps Independent to `None` deliberately, since independence is the absence of affiliation, not a party.

Phase 3 must add the historical tokens and mint their party Organizations (today only 2 exist, carrying *modern brand* acronyms `WAGOP`/`WA DEMS` — these defunct parties have only the source's abbreviations), or explicitly decline them with a tally — never fold them to `None` by default. Naming, acronym semantics and defunct-party lifecycle are **power-map#442**; the local work is **#227**.

## Phasing

Ascending write-risk. Each phase ships independently and is separately valuable. Tracked as **#225** (Phase 1), **#226** (Phase 2), **#227** (Phase 3a, parties), **#228** (Phase 3b), **#229** (Phase 3c), **#230** (Phase 3d, pre-1965), **#231** (Phase 4); the dependency map and external gates live on the #219 epic.
