...
- **Committees retire.** Id 31639 (2021-22 "Commerce & Gaming" → 2023-24 "Regulated
  ...
  presence diffing, not exposed as a field — ties to `LifecycleMixin` (#38/#42).
...
8. **Active-in-biennium status (P1a unrepresented; resolved by Open Q 5).** A committee that exists in one biennium and not the next leaves a **P1a** durable Org row with no presence marker. `GetActiveCommittees` is current-only (no biennium arg). **Resolved (Open Q 5):** usa-wa does **not** autonomously retire on biennium absence — current-only `GetActiveCommittees` cannot distinguish *dormant* (statutory committee that didn't meet) from *abolished*, and per-biennium presence is a P1b **Assignment** concern, not an Org-retirement one. Inactivation is **PM-curated**: PM's "inactive" signal is `archived_at` on the org read model, which the sidecar mirrors onto the local `archived_at` tombstone (#40/#42) so an inactivated committee drops from live reads. "Was X active in biennium Y?" is answered by the session-scoped Assignment layer (P1b).
...
5. ✅ **Committee biennium-history modeling — one durable Org, PM-curated name timeline (no fork).** *Resolved (2026-06-24, #40 + power-map#239; an earlier draft of this answer wrongly assumed PM forks-on-rename — corrected here):* Power Map does **not** mint a new Org on rename. A WSL committee rename is **one durable PM Org for the committee's whole life**; `org_wa_legislature_committee_id=Id` stays anchored to it (a documented PM public-API invariant — per-epoch identifiers are explicitly rejected as they'd break PM's identifier-uniqueness and invert its merge-rebrands tooling). usa-wa mirrors this:
   ...
   - **Inactivation is PM-curated, mirrored read-side.** usa-wa does **not** drive retirement to PM (the observation/producer path has no active/inactive field; only an admin endpoint does). PM's inactive signal is `archived_at` on the org read model; the sidecar org descriptor mirrors it onto the local `archived_at` tombstone (#40/#42), so an inactivated committee drops from live reads. Biennium *presence* is the P1b Assignment layer's job — usa-wa never decides dormant-vs-abolished from current-only `GetActiveCommittees`.
