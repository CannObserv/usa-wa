...
Committee membership + party Roles use the generic `member` `role_type` ([power-map#269](https://github.com/CannObserv/power-map/issues/269), `expects_jurisdiction=false`), matched on `(org, title)`. Neither PM dependency blocks P1b (both accepted/implemented).
