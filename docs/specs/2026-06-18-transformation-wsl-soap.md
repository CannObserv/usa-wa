...
- **Chamber (House) membership** — `Person → member Role` (`role_type="member"`, [power-map#269](https://github.com/CannObserv/power-map/issues/269)) on the House org (captures chamber affiliation this cut). The precise **House seat** (`state_representative`, `qualifier` Position 1/2) is **deferred to #69** — WSL has no Position source, and PM's create path mints an unseeded structural tuple, so a NULL-qualifier House seat would create a spurious PM seat. District (on the seat) arrives with #69; chamber + party + committee membership are captured now.
...
- **Committee membership** — `Person → member Role` (`role_type="member"`) on the committee Org (membership-only; chair/vice has no WSL source).

Committee + House-chamber membership Roles use the generic `member` `role_type` (power-map#269, `expects_jurisdiction=false`), matched on `(org, title)`. Neither PM dependency blocks P1b (both accepted/implemented).
