...
| GET | `/health/datasets` | Publication health (#311): the catalog's heartbeat — `checked_at`, `checked_age_seconds`, `stale_after`, `stale` (#386; `null` when the catalog declares no deadline) — then per-dataset `tier`, latest version, rows and `age_seconds` since that version minted; `published: false` before the first publish. What the heartbeat does and does not cover: [PIPELINE-PUBLICATION.md](PIPELINE-PUBLICATION.md#the-catalogs-heartbeat-386). The pipeline-era successor to `/health/sync`. `tier` is what says whether a dataset is the subscriber contract — `conformed` — or `staging` / `internal` / `cutover`; the tiers are defined in [PIPELINE-PUBLICATION.md](PIPELINE-PUBLICATION.md#publication-311). |
